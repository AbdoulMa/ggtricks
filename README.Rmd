...
You have a lot of functions to make sector charts where circle is split along its circumference,
therefore each section is proportional to value it represents.
...
Of course, you can choose, to customize the categories labels by setting `axis.text` in `theme_*()` function. But the need may come to add labels at series of circles boundary positions. There comes `geom_series_text()` function.

```{r series-circles-with-text}
prod_df |>
  ggplot() +
  geom_series_circles(aes(index, good, fill = good), color = "black", linewidth = 2.5) +
...
```{r series-circles-init-angles, eval=FALSE}
index_df <- tribble(
  ~article, ~index,
  "Plate beef", 187,
  "Bacon", 215,
...
![Series of circles Init angles](man/figures/series_circles_init_angle_montage.png)
...
```{r series-circles-combination, fig.show='hide'}
usa_trades <- tribble(
...
```{r series-circles-combination-saving, include=FALSE}
ggsave("man/figures/series_circles_combination.png", width = 9.5, height = 5, dpi = 300, device = ragg::agg_png)
```

![Series of circles combination](man/figures/series_circles_combination.png)
...
Maybe, you want a specific category to drive the slices positions rather than the category with the maximum value ? 
This is where comes the `spotlight_cat` parameter to define the driving category. Also here you can combine the  `spotlight_cat` parameter value with `spotlight_position` to specify its position.

```{r pie-cat-spotlighted}
my_df |>
  ggplot() +
  geom_pie(aes(cat = cat, val = val, fill = cat),
...
As I know that it can be difficult to know the coordinates of the center positions of the category slices, I define a  default `label` mapping which will place the provided labels at this position. When `label` mapping is defined, you can define `labels_with_tick` 
...
Donut is just pie with a hole in it. There are two parameters `r1` and `r2` to define the thickness of the donut. 
...
Note here that there are no `spotlight_max`, `spotlight_cat`  parameters, since we are not drawing a complete circle (but theoretically you can, if you set `slice_angle` to 360, which means a `pie`.)
...
Soon, I  will post more detailed examples on the package website: <https://abdoulma.github.io/ggtricks/>. 
...
Since we are using `coord_equal()`, you won't be able to set `scales` parameter, which I strongly suspect you to try to do. So for the moment, I advise you not to do so.
However, I will provide some tips to go through those restrictions on package website <https://abdoulma.github.io/ggtricks/>
